  ...
  Future<bool> uploadFile(String filePath, String directory) async {
    if (settingsController.s3BucketName.value.isNotEmpty) {
      ...
      String destinationPath =
          "${path.dirname(filePath)}/upload/${path.basename(filePath)}.gz";
      ...
      if (await currentFile.exists()) {
        ...
        ImageData imageData = ImageData(
            path.basename(copiedFile.path), copiedFile.path,
            imageUploadFolder:
                "${settingsController.s3DestDir.value}/$directory");

        result = await Aws3Bucket.upload(
            settingsController.s3BucketName.value!,
            AwsRegion.US_EAST_1,
