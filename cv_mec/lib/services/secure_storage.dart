...
class SecureStorage {
  ...
  Future<String> getDeviceID() async =>
      await _storage.read(key: _keyDeviceID) ?? Future.value(_startDeviceID);
  ...
  Future<String> getS3AccessKey() async =>
      await _storage.read(key: _keyS3Accesskey) ??
      Future.value(_startS3AccessKey);
  Future<String> getS3SecretKey() async =>
      await _storage.read(key: _keyS3SecretKey) ??
      Future.value(_startS3SecretKey);
  Future<String> getS3BucketName() async =>
      await _storage.read(key: _keyS3BucketName) ??
      Future.value(_startS3BucketName);
  Future<String> getS3Region() async =>
      await _storage.read(key: _keyS3Region) ?? Future.value(_startS3Region);
  Future<String> getS3DestDir() async =>
      await _storage.read(key: _keyS3DestDir) ?? Future.value(_startS3DestDir);

  Future setS3AccessKey(String s3AccessKey) async =>
      await _storage.write(key: _keyS3Accesskey, value: s3AccessKey);
  Future setS3SecretKey(String s3SecretKey) async =>
      await _storage.write(key: _keyS3SecretKey, value: s3SecretKey);
  Future setS3BucketName(String s3BucketName) async =>
      await _storage.write(key: _keyS3BucketName, value: s3BucketName);
  Future setS3Region(String s3Region) async =>
      await _storage.write(key: _keyS3Region, value: s3Region);
  Future setS3DestDir(String s3DestDir) async =>
      await _storage.write(key: _keyS3DestDir, value: s3DestDir);
