...
class TimManager {
  ...
  Map<String, TravelerInformation> storedTims = <String, TravelerInformation>{}; // Store all Tims that have been received
  ...
  void addOrUpdate(TravelerInformation tim, String asn1){
    ...
    if(tim.packetID != null){
      ...
      if(storedTims.containsKey(packetID)){
        if(storedTims[packetID]!.msgCnt.msgCount < tim.msgCnt.msgCount){
          removeTim(packetID);
          ...
          if(isDataFrameTimeActive(dataFrame)){
            for(GeometryDirection geometry in dataFrameGeometry.geometry){
              ...
              if(geometryService.isPointInPolygon(geometry.geometry, longitude, latitude)){
                if(geometry.direction != null && isDirectionInHeadingSlice(heading, geometry.direction!)){
                  anyActiveZone = true;
                }
              }else{
                dataFrameGeometry.active = false;
                dataFrameGeometry.shown = false; // Reset message being shown when the user leaves the specified zone. They can receive the message again if they enter in a valid direction.
              }
            }
  ...
  List<DataFrameGeometry> getActiveTimGeometry(){
    ...
    print("Stored TIMS: ${storedTims.length}");
  ...
  bool isDataFrameTimeActive(TravelerDataFrame dataFrame){
    ...
    DateTime startYear = DateTime.utc(2024);
    DateTime startTime = startYear.add(Duration(minutes: dataFrame.startTime.minuteOfTheYear));
    DateTime endTime = startTime.add(Duration(minutes: dataFrame.durationTime.minutesDuration));



    // print("START TIME, END TIME: $startYear, $startTime, $now, $endTime");

    return startTime.isBefore(now) && endTime.isAfter(now); // Allow all TIM messages to be active for now.
  }
  ...
  bool isDirectionInHeadingSlice(double direction, HeadingSlice headingSlice){
    ...
    return 
      direction >= 0 && direction < 22.5 && headingSlice.from000_0to022_5degrees ||
