...
class SpatManager {
  ...
  LightChangeTime? getNextLaneTimeChange(
      int intersectionId, int signalGroup, DateTime now) {
      ...
      if (state.moy != null) {
        for (MovementState movement in state.states.movementList) {
          if (movement.signalGroup.signalGroupID == signalGroup) {
            for (MovementEvent movementEvent
                in movement.state_time_speed.movementEventList) {
              if (movementEvent.timing != null) {
                return LightChangeTime(movementEvent.timing!,
                    state.getUtcTime(), movementEvent.eventState);
              }
            }
          }
        }
