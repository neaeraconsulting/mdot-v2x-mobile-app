...
class GeoMap {
    ...
    if (intersectionGeometry.laneWidth != null) {
      for (GenericLane lane in intersectionGeometry.laneSet.laneList) {
        List<LatLng> laneCoordinates =
            _geometryService.getLatLngCoordinatesFromNodeSetXY(
                lane.nodeList.nodeListXY as NodeSetXY,
                intersectionGeometry.refPoint);

        Geometry? laneBoundingGeometry =
            _geometryService.getGeometryFromNodeListXY(
                lane.nodeList,
                intersectionGeometry.refPoint,
                intersectionGeometry.laneWidth!.getLaneWidthMeters());
      ...
      mapBoundingBox = _geometryService
          .calculateMultiGeometryBoundingBox(laneBoundaries.values.toList());
    }
    ...
    for (GenericLane lane in intersectionGeometry.laneSet.laneList) {
      if (lane.connectsTo != null) {
        ...
        for (Connection connection in lane.connectsTo!.connectsTo) {
          ...
          List<LatLng> coordinates = calculateLaneConnectionCoordinates(
              ingressLaneId, connectId, intersectionGeometry.refPoint);
          ...
          if (lightLocations.containsKey(coordinates.first)) {
            ...
            lightLocations[coordinates.first] =
                RenderLightLocation(coordinates.first, signalGroups);
          }
        }
      }
    }
  ...
  List<LatLng> calculateLaneConnectionCoordinates(
      int firstLaneId, int secondLaneId, Position3D refPoint) {
    if (laneSegments.containsKey(firstLaneId)) {
      if (laneSegments.containsKey(secondLaneId)) {
        ...
        double angle =
            getAngle(leadInPoint, startPoint, endPoint, leadOutPoint);

        if (angle > 45) {
          LatLng? controlPoint = calculateLineIntersection(
              leadInPoint, startPoint, endPoint, leadOutPoint);

          if (controlPoint != null) {
            List<LatLng> points =
                calculateBezier(startPoint, endPoint, controlPoint);
  ...
  double getAngle(LatLng p1, LatLng p2, LatLng p3, LatLng p4) {
    ...
    double theta = acos(
        (dx1 * dx2 + dy1 * dy2) / (getLength(dx1, dy1) * getLength(dx2, dy2)));
  ...
  LatLng? calculateLineIntersection(
    ...
    double xNumerator =
        (p1.longitude * p2.latitude - p1.latitude * p2.longitude) *
                (p3.longitude - p4.longitude) -
            (p1.longitude - p2.longitude) *
                (p3.longitude * p4.latitude - p3.latitude * p4.longitude);
    double yNumerator =
        (p1.longitude * p2.latitude - p1.latitude * p2.longitude) *
  ...
  List<LatLng> calculateBezier(LatLng ingress, LatLng egress, LatLng control) {
    ...
    for (double i = 0; i <= interpolationPoints; i++) {
      ...
      double latitude = pow((1 - t), 2) * ingress.latitude +
          2 * (1 - t) * t * control.latitude +
          pow(t, 2) * egress.latitude;
      double longitude = pow((1 - t), 2) * ingress.longitude +
          2 * (1 - t) * t * control.longitude +
          pow(t, 2) * egress.longitude;
