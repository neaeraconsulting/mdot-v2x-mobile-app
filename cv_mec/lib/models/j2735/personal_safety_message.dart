...
import 'package:cv_mec/models/j2735/animal_type.dart';
import 'package:cv_mec/models/j2735/attachment.dart';
import 'package:cv_mec/models/j2735/attachment_radius.dart';
...
import 'package:cv_mec/models/j2735/number_of_participants_in_cluster.dart';
import 'package:cv_mec/models/j2735/path_history.dart';
import 'package:cv_mec/models/j2735/path_prediction.dart';
import 'package:cv_mec/models/j2735/personal_assistive.dart';
import 'package:cv_mec/models/j2735/personal_cluster_radius.dart';
import 'package:cv_mec/models/j2735/personal_crossing_in_progress.dart';
import 'package:cv_mec/models/j2735/personal_crossing_request.dart';
import 'package:cv_mec/models/j2735/personal_device_usage_state.dart';
import 'package:cv_mec/models/j2735/personal_device_user_type.dart';
import 'package:cv_mec/models/j2735/position_3d.dart';
import 'package:cv_mec/models/j2735/positional_accuracy.dart';
import 'package:cv_mec/models/j2735/propelled_information.dart';
import 'package:cv_mec/models/j2735/public_safety_and_road_worker_activity.dart';
import 'package:cv_mec/models/j2735/public_safety_directing_traffic_sub_type.dart';
import 'package:cv_mec/models/j2735/public_safety_event_responder_worker_type.dart';
import 'package:cv_mec/models/j2735/temporary_id.dart';
import 'package:cv_mec/models/j2735/user_size_and_behavior.dart';
...
class PersonalSafetyMessage {
  ...
  PersonalSafetyMessage.fromC(C.PersonalSafetyMessage c_personalSafetyMessage) {
    basicType =
        PersonalDeviceUserType.values[c_personalSafetyMessage.basicType];
    ...
    if (c_personalSafetyMessage.accelSet.address != 0) {
      accelSet =
          AccelerationSet4Way.fromC(c_personalSafetyMessage.accelSet.ref);
    }
    ...
    if (c_personalSafetyMessage.pathPrediction.address != 0) {
      pathPrediction =
          PathPrediction.fromC(c_personalSafetyMessage.pathPrediction.ref);
    }

    if (c_personalSafetyMessage.propulsion.address != 0) {
      propulsion =
          PropelledInformation.fromC(c_personalSafetyMessage.propulsion.ref);
    }

    if (c_personalSafetyMessage.useState.address != 0) {
      useState = PersonalDeviceUsageState.fromBitString(
          c_personalSafetyMessage.useState.ref);
    }

    if (c_personalSafetyMessage.useState.address != 0) {
      crossRequest =
          PersonalCrossingRequest(c_personalSafetyMessage.crossRequest.value);
    }

    if (c_personalSafetyMessage.crossState.address != 0) {
      crossState =
          PersonalCrossingInProgress(c_personalSafetyMessage.crossState.value);
    }

    if (c_personalSafetyMessage.clusterSize.address != 0) {
      clusterSize = NumberOfParticipantsInCluster
          .values[c_personalSafetyMessage.clusterSize.value];
    }

    if (c_personalSafetyMessage.clusterRadius.address != 0) {
      clusterRadius =
          PersonalClusterRadius(c_personalSafetyMessage.clusterRadius.value);
    }

    if (c_personalSafetyMessage.eventResponderType.address != 0) {
      eventResponderType = PublicSafetyEventResponderWorkerType
          .values[c_personalSafetyMessage.eventResponderType.value];
    }

    if (c_personalSafetyMessage.activityType.address != 0) {
      activityType = PublicSafetyAndRoadWorkerActivity.fromBitString(
          c_personalSafetyMessage.activityType.ref);
    }

    if (c_personalSafetyMessage.activitySubType.address != 0) {
      activitySubType = PublicSafetyDirectingTrafficSubType.fromBitString(
          c_personalSafetyMessage.activitySubType.ref);
    }

    if (c_personalSafetyMessage.assistType.address != 0) {
      assistType = PersonalAssistive.fromBitString(
          c_personalSafetyMessage.assistType.ref);
    }

    if (c_personalSafetyMessage.sizing.address != 0) {
      sizing =
          UserSizeAndBehavior.fromBitString(c_personalSafetyMessage.sizing.ref);
    }
    ...
    if (c_personalSafetyMessage.attachmentRadius.address != 0) {
      attachmentRadius =
          AttachmentRadius(c_personalSafetyMessage.attachmentRadius.value);
    }
