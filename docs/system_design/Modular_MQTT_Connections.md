...
The CV-MEC application is designed to connect to multiple different [MQTT](https://mqtt.org/ "https://mqtt.org/") systems concurrently. This enables the app to be used for testing out multiple different data sources. As of version 67 of the CV-MEC application, the CV-MEC application is not dependent on any one MQTT Broker. However, in order to send or receive any messages at least one broker must be enabled in the settings menu. It is recommend that as new brokers are introduced to the application, additional configuration is added to the settings menu in order to streamline testing. CV-MEC also supports sending and receiving on multiple brokers concurrently. This allows the CV-MEC app to communicate effectively even with apps that are only capable of listening on a single broker. For example the [ISS Traffic Auth](https://www.ghsiss.com/ta-mobile/ "https://www.ghsiss.com/ta-mobile/") app can only communicate using the ISS MQTT Platform, and the Arizona Drive App (not yet, publicly available). However CV-MEC can communicate with both 3rd party applications.

To facilitate long term maintainability with multiple brokers, Version 67 of the CV-MEC application restructures the existing MQTT connections into multiple independent “agents“. Each agent inherits core features from a common [mqtt_agent](https://github.com/usdot-fhwa-stol/v2x-mobile-app/blob/develop/cv_mec/lib/models/mqtt/mqtt_agent.dart "https://github.com/usdot-fhwa-stol/v2x-mobile-app/blob/develop/cv_mec/lib/models/mqtt/mqtt_agent.dart")  parent which allows the agent to be managed by the [mqtt_agent_manager](https://github.com/usdot-fhwa-stol/v2x-mobile-app/blob/develop/cv_mec/lib/models/mqtt/mqtt_agent_manager.dart "https://github.com/usdot-fhwa-stol/v2x-mobile-app/blob/develop/cv_mec/lib/models/mqtt/mqtt_agent_manager.dart"). Programmatically, defining an agent requires three critical things to be defined.  
